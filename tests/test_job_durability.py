...
async def test_a_job_survives_the_server_that_started_it(tmp_path, fake_codex, monkeypatch):
    ...
    try:
        # The worker takes worker.lock only after its imports; an unowned reader before
        # that reads a young job as failed (pontonier liveness rule) — recorded as a
        ...
        # "Restart": a different Python process (a fresh _PROCESS_OWNER) reads the same
        # store.
        ...
        # Idempotent on a terminal job (returns its status unchanged): a no-op on the
        # success path, a guaranteed kill if any assertion above failed.
        store.cancel(str(tmp_path), job_id)
