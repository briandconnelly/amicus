"""AMICUS_KIMI_* resolution: retired names, isolation default, refused extra args, versions."""
...
def test_namespace_declares_the_six_settings_with_retired_twins():
    names = kc.ENV.names()
    assert names == (
        "AMICUS_KIMI_BIN",
    ...
    assert all(kc.ENV.var(name).legacy == () for name in names)
    assert kc.ENV.var("AMICUS_KIMI_BIN").removed == ()
    assert kc.ENV.var("AMICUS_KIMI_MODEL").removed == ("MOONBRIDGE_MODEL",)
...
def test_retired_names_are_reported_and_never_read():
    """#176: the MOONBRIDGE_* names are tombstones since 0.4.0."""
    cfg = kc.load_config({"MOONBRIDGE_MODEL": "k3", "MOONBRIDGE_ISOLATION": "ignore-skills"})
    assert cfg.model is None and cfg.isolation == "inherit" and cfg.errors == ()
    assert any("MOONBRIDGE_MODEL is set but not read" in w for w in cfg.warnings)
    assert any("MOONBRIDGE_ISOLATION is set but not read" in w for w in cfg.warnings)
    pair = kc.load_config({"AMICUS_KIMI_MODEL": "a", "MOONBRIDGE_MODEL": "b"})
