...
async def test_backends_reports_codex_ready_live(live_codex):
    ...
    # minor in the built-in SUPPORTED_VERSIONS will do; kimi's gate pins the newest one
    # instead, at the cost of blocking evidence on every CLI upgrade.
    assert_codex_version_was_checked(entry["status"])
...
async def test_review_changes_live(live_codex, tmp_path):
    ...
    async with Client(_app()) as c:
        res = await c.call_tool(
            ...
            {"backend": "codex", "workspace_root": str(tmp_path), "timeout_seconds": 150},
            raise_on_error=False,
        )
    body = res.structured_content
    assert body["ok"] is True, body.get("error", {}).get("code")
...
async def test_unknown_model_is_an_envelope_not_an_exception_live(live_codex, tmp_path):
    ...
    assert body["error"]["code"] in ("nonzero_exit", "cli_contract_changed", "invalid_model")
