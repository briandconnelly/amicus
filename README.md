...
**Status:** milestone M3 (Kimi).
`amicus_consult`, `amicus_review_changes`, `amicus_delegate`, their `_async` twins, both dry runs and the five `amicus_job_*` tools work for `backend="codex"` and `backend="kimi"`; every paid call runs in a detached worker and records a job (`meta.job_id`), and `idempotency_key` dedups an `_async` retry.
Claude Code (M4) still returns `backend_unavailable`; `amicus_adversarial_review(_async)` returns `not_implemented` until M4; `task=True` wiring lands in M5.
