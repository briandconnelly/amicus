...
- **Kimi keeps its own copy.** The kimi CLI writes the whole prompt and answer of every run to
  its own session store (by default `~/.kimi-code/sessions`). amicus found no way to switch that
  off and does not delete it, and none of the retention controls above reach it; Codex and Claude
