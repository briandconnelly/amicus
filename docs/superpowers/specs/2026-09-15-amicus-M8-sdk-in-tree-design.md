...
- **Logging.**
  ...
  The job worker did not call `obs.configure`, before or after M8, so there SDK records fell through to `logging.lastResort` and the worker's `stderr.log`.
  M8 left that routing as it was, and #128 later changed it so the worker configures before anything else it does.
