...
2. Decide the PyPI action: `codex-in-claude`'s classifiers currently declare `Development Status :: 4 - Beta` (`pyproject.toml`).
   Change that classifier to `Development Status :: 7 - Inactive` and publish a final release so the change reaches PyPI; do not yank any existing release. Yanking would not actually break the pinned install (PEP 592: a yanked release is still selected when a requirement pins that exact version, so `codex-in-claude==0.22.0` in `.mcp.json` and the plugin marketplace's launch from that exact pin would keep resolving). The reason to leave releases un-yanked is instead to preserve discoverability and continued support for anyone still depending on `codex-in-claude` directly (outside the pinned install path) during the deprecation window — a yank signals "broken, do not use" on PyPI's own listing, which is not the message intended here.
...
2. Decide the PyPI action: `claude-in-codex`'s classifiers currently declare `Development Status :: 3 - Alpha` (`pyproject.toml`), one stage earlier than the other two siblings.
   Change the classifier to `Development Status :: 7 - Inactive` and publish a final release; do not yank existing PyPI releases. Here the pin isn't even a reason: both `.mcp.json` (root) and `plugins/claude-in-codex/.mcp.json` install from the git tag `git+https://github.com/briandconnelly/claude-in-codex.git@v0.9.0`, not from PyPI, so yanking a PyPI release would be irrelevant to that install path regardless of PEP 592 semantics. As with `codex-in-claude`, the reason to leave releases un-yanked is preserving discoverability and continued support on PyPI for anyone who does depend on it from there, matching the intended deprecation policy rather than a breakage that would not occur.
