...
The vocabularies move: `_REASONS_DESC`, the reason definitions of `_LISTS_DESC`, `_OMISSION_REASONS_DESC`, `_UNTRACKED_DESC`, `_REDACTION_DESC`, `_WITHHELD_DESC` and `_MASKED_DESC`, 6,231 bytes on the `all` wire.
The nested descriptions they sat on are removed, not replaced, and one pointer sentence is appended to each parent disclosure field's retained guard: `findings_diagnostics` on four tools, `lists_diagnostics` on three, `coverage` on three including the dry run, ten pointers in all, each about 100 bytes.
...
Measured with the wire instrument on a throwaway edit of `2ab6a8c` that applied exactly that strategy, `tools/list` went from 112,105 to 106,871 bytes on `all` and `claude` and from 112,113 to 106,879 on `codex-kimi`, a saving of 5,234 bytes on every profile, 4.7% of the catalog, before the roughly 40 bytes the new `include_schemas` value adds; that is a projection to be repeated on the implementation branch, not acceptance evidence.
The change ships only if the measured reduction on every profile is at least 4,500 bytes, and only if the host scenarios in the #247 plan show an agent with no skill loaded reading each disclosure correctly after following the pointer: a list lost on an otherwise clean result, a finding that lost content at `dropped: 0`, a `number_stringified` normalization that lost nothing, a `focused` review whose coverage withheld nothing, and a `redacted` reason beside a null `redaction` because the byte cap cut the affected content.
Every vocabulary that moves is covered by a scenario that turns on it, because the retained guards do not explain those distinctions.
...
- `tools/list` on `all` is expected to drop by about 5,200 bytes, 5,234 measured on the throwaway edit less the enum value; that is modest, about 1,300 tokens per connection on a preloading host, and it is the whole of the repeated prose, since the remaining output-schema bytes are structure MCP cannot share across tools; the delivered figure is recorded in `tests/test_discovery_cost.py` with `MEASURED` and `BUDGET` moved together.
