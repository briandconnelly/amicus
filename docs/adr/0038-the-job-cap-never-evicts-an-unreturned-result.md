...
Its repair lists the workspace's jobs with `amicus_job_list`.
Running jobs count toward the cap, so the cap now bounds how many records a workspace holds rather than letting running jobs overshoot it.
