...
- **Evidence:** S7 assertion 3 failed on both hosts.
  ...
  The model explained that the diff is never applied and that the task text goes to the provider raw, but it never attributed the approval to `claude` being enabled."
  Codex CLI: `docs/host-captures/install-smoke/codex/0.153.4/transcript.md`, "It reported that approval was required and that `client.py` was unchanged, and it never connected the approval to `claude` being enabled."
