...
CAPABILITY_SUMMARY = (
    ...
    "unkeyed sync call past its deadline is terminated and its partial work lost, while a "
    "keyed one (idempotency_key) leaves its run going and its repair says how to fetch it. "
    ...
    "field (isError is set on it too). Cancelling a task cancels its job unless the call "
    "carried an idempotency_key, when the shared job survives. A task result "
