...
TOOL_DETAILS: dict[str, dict[str, Any]] = {
    ...
    "amicus_job_consume_result": {
        "cost": "free",
        "backends": list(BACKEND_IDS),
        "use_when": "Fetch a finished job's envelope or terminal error and delete its record.",
        ...
        "state_changed or delete_failed. A failed, cancelled or timed-out job returns its "
        "terminal error, and meta.consume reports its discard too. A corrupt or "
        "incompatible record is kept.",
        "error_codes": [
            "job_not_found",
            "job_running",
            "job_failed",
...
async def capabilities_payload(
    ...
    caps = CapabilitiesResult(
        ...
        annotations_reading=(
            ...
            + "A paid call never deletes a tracked, deliverable result amicus has not yet "
            "returned to make room for its own job record: the per-workspace cap "
            "(AMICUS_JOB_MAX_COUNT) evicts only expired records, terminal errors, results "
            "already returned once, results in an older result format no server can deliver, "
            "and records written before delivery was tracked; when only running jobs and "
            "tracked, deliverable unreturned results remain it refuses the call pre-spend with "
            "job_cap_reached. Returned once means amicus handed the result back, not that the "
            "client received it. "
