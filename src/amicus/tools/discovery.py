...
async def capabilities_payload(
    ...
    caps = CapabilitiesResult(
        ...
        annotations_reading=(
            ...
            + "A paid call never deletes a deliverable result amicus has not yet returned to "
            "make room for its own job record: the per-workspace cap (AMICUS_JOB_MAX_COUNT) "
            "evicts only expired records, terminal errors, results already returned once, "
            "results in an older result format no server can deliver, and records written "
            "before delivery was tracked; when only running jobs and unreturned results "
            "remain it refuses the call pre-spend with job_cap_reached. Returned once means "
            "amicus handed the result back, not that the client received it. "
