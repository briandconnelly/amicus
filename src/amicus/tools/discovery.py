...
async def capabilities_payload(
    ...
    caps = CapabilitiesResult(
        ...
        annotations_reading=(
            ...
            + "A paid call never deletes a tracked result amicus has not yet returned to make "
            "room for its own job record: the per-workspace cap (AMICUS_JOB_MAX_COUNT) evicts "
            "only expired records, terminal errors, results already returned once and records "
            "written before delivery was tracked, whatever release wrote them; when only "
            "running jobs and tracked, unreturned results remain it refuses the call pre-spend "
            "with job_cap_reached. Returned once means amicus handed the result back, not that "
