...
2. Read the handle's `status`. If it is already terminal, go straight to step 4. Otherwise wait at
   least the returned `poll_after_ms`, then call `amicus_job_status` with the same `job_id` and
   the same absolute `workspace_root`.
