...
it accepts, so any other pairing is rejected at the boundary, pre-spend, as `invalid_arguments`
with `details.allowed_values`; `feature_unsupported` remains only for a third-party backend plugin
