...
- **Breaking (`FINGERPRINT` `schema-30`).** Every `amicus_job_consume_result` surface now says
  ...
  stored envelope, so a consume returns its terminal error with no `meta.consume` and deletes
  nothing, and `amicus_job_list` still lists the record afterwards. The tool description,
  discovery's `use_when` and `returns`, `/amicus:jobs` and the skill's options-and-errors
  reference all said that `meta.consume.discard_outcome` reports what the store did, and none
  of them named this case. Behavior and `RESULT_FORMAT` are unchanged. Deleting such a record
  needs an atomic compare-and-delete from pontonier (briandconnelly/pontonier#31), because a
  record read as `failed` can turn `done` after it is read.
